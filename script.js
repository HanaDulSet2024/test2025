async function fetchNovelContent(url, episodeNumber) {
    ...
    let cleanedText = cleanText(content.innerHTML);

    // Add separator and episode number
    const separator = "\n\n" + "--------------------" + "\n" + `제 ${episodeNumber}화` + "\n\n";
    cleanedText += separator;
...
async function downloadNovel(title, episodeLinks, startEpisode) {
    ...
    for (let i = startingIndex; i >= 0; i--) {
        ...
        let episodeContent = await fetchNovelContent(episodeUrl, episodeNumber);
...
새 탭에서 해당 페이지에 접속하여 캡챠를 풀고, 확인을 눌러주세요.`);
            ...
            if (userConfirmed) {
                ...
                episodeContent = await fetchNovelContent(episodeUrl, episodeNumber);
                if (!episodeContent) {
